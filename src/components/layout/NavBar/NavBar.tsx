...
import { 
  ChevronDown, Headset, Heart, Search, 
  ShoppingCart, User, UserCircle, LogOut, Settings 
} from "lucide-react";
...
const pathname = usePathname();
  return (
    <>
      <NavHeader/>
      <header className={cn("sticky top-0 z-40 w-full border-b bg-background/95 backdrop-blur supports-backdrop-filter:bg-background/60 shadow-sm", className)}>
        <div className="container mx-auto px-4">
          <nav className="flex items-center justify-between h-16 lg:h-20 gap-4">
            
            {/* Logo */}
            <Link className="shrink-0 transition-opacity hover:opacity-90" href="/">
              <Image
                alt="Yassify"
                width={161}
                height={31}
                  ...
                  <NavigationMenuContent>
                    <ul className="grid w-62.5 gap-1 p-4 bg-popover rounded-md shadow-md border">
                      {categories.length > 0 ? categories.map((cat, i) => (
                        ...
                        <p className="text-xs text-muted-foreground p-2 text-center">Loading categories...</p>
                      )}
                    </ul>
                  </NavigationMenuContent>
                </NavigationMenuItem>
            ...
            <div className="flex items-center gap-1 lg:gap-3">
              ...
              >
                <div className="w-9 h-9 rounded-full bg-green-50 dark:bg-green-900/20 flex items-center justify-center">
                  <Headset size={18} className="text-green-600 dark:text-green-500" />
                </div>
              ...
              {/* Wishlist */}
              <Link href="/wishlist" className="relative p-2 rounded-full hover:bg-muted transition-colors group">
                <Heart size={22} className="text-foreground/80 group-hover:text-red-500 transition-colors" />
                {/* {count > 0 && (
              ...
              {/* Cart */}
              <Link href="/cart" className="relative p-2 rounded-full hover:bg-muted transition-colors group">
                <ShoppingCart size={22} className="text-foreground/80 group-hover:text-green-600 transition-colors" />
                {/* {numOfCartItems > 0 && (
              ...
              <Separator orientation="vertical" className="h-6 mx-1 hidden sm:block" />
                ...
                <Button asChild size="sm" className="hidden sm:flex rounded-full bg-green-600 hover:bg-green-700 text-white gap-2 px-6">
                  <Link href="/login">
                    <User size={16} />
                    Sign In
                  </Link>
