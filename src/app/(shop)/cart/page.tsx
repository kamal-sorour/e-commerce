...
import ViewToggle, {
  ViewType,
} from "@/components/shared/ViewToggle/ViewToggle";
...
export default function CartPage() {
  ...
  const total = cartDetails?.data?.data?.totalCartPrice
    ? cartDetails.data.data.totalCartPrice + shippingCost
              ...
              <div className="lg:col-span-1">
                <Card className="bg-white dark:bg-slate-900 rounded-3xl border-slate-200 dark:border-slate-800 shadow-lg overflow-hidden sticky top-24">
                  ...
                  <div className="p-6 space-y-6">
                    ...
                    <Button
                      variant="outline"
                      className="w-full h-14 rounded-2xl border-dashed border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-400 hover:border-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-950/30 hover:text-emerald-600 dark:hover:text-emerald-400 transition-colors font-bold gap-2"
                    >
                    ...
                    <Button
                      asChild
                      className="w-full h-14 rounded-2xl font-bold text-base bg-slate-900 hover:bg-slate-800 dark:bg-emerald-600 dark:hover:bg-emerald-700 text-white shadow-lg active:scale-95 transition-all gap-2"
                    >
                      <Link href={`/checkout?id=${cartDetails?.data.cartId}`}>
                    ...
                    <div className="flex items-center justify-center gap-4 pt-2">
                      ...
                      <div className="flex items-center gap-1.5 text-xs font-bold text-slate-400 dark:text-slate-500">
                        <Truck className="text-blue-500" size={14} /> Fast
                        Delivery
                      </div>
                    </div>
                  </div>
        ...
        <section className="min-h-screen bg-slate-50 dark:bg-slate-950 flex items-center justify-center px-4 transition-colors duration-500">
          <div className="max-w-md text-center p-8">
            ...
            <p className="text-slate-500 dark:text-slate-400 mb-8 font-medium leading-relaxed">
              ...
              our premium products and start shopping!
            </p>
            <Button
              asChild
              className="h-14 px-8 rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold shadow-lg shadow-emerald-600/20 transition-all active:scale-95 gap-2"
            >
              <Link href="/products">
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {["Electronics", "Fashion", "Home", "Beauty"].map((cat) => (
                  <Link
                    key={cat}
                    href="/categories"
                    className="px-4 py-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 hover:border-emerald-500 hover:text-emerald-600 dark:text-slate-300 dark:hover:text-emerald-400 rounded-full text-sm font-bold transition-colors"
                  >
                    {cat}
                  </Link>
                ))}
